...
pesosPrimeiroNeuronioCamadaEntrada = np.array([uniform(-1, 1) for i in range(0, 4)])
pesosSegundoNeuronioCamadaEntrada = np.array([uniform(-1, 1) for i in range(0, 4)])

pesosPrimeiroNeuronioCamadaOculta = np.array([uniform(-1, 1) for i in range(0, 2)])
pesosSegundoNeuronioCamadaOculta = np.array([uniform(-1, 1) for i in range(0, 2)])

pesosNeuronioDeSaida = np.array([uniform(-1, 1) for i in range(0, 2)])
...
class RedeNeural():
    ...
    def feedforward(self):
        ...
        self.saidaPrimeiroNeuronioCamadaEntrada = round(
            self.tangenteHiperbolica(np.sum(self.entradas * self.pesosPrimeiroNeuronioCamadaEntrada)), 6)

        self.saidaSegundoNeuronioCamadaEntrada = round(
            self.tangenteHiperbolica(np.sum(self.entradas * self.pesosSegundoNeuronioCamadaEntrada)), 6)
        ...
        self.saidaPrimeiroNeuronioCamadaOculta = round(self.tangenteHiperbolica(np.sum(np.array(
            [self.saidaPrimeiroNeuronioCamadaEntrada,
             self.saidaPrimeiroNeuronioCamadaEntrada]) * self.pesosPrimeiroNeuronioCamadaOculta)), 6)

        self.saidaSegundoNeuronioCamadaOculta = round(self.tangenteHiperbolica(
            np.sum(np.array([self.saidaPrimeiroNeuronioCamadaEntrada,
                             self.saidaSegundoNeuronioCamadaEntrada]) * self.saidaSegundoNeuronioCamadaEntrada)), 6)
...
while not gameOver:
    ...
    with open('dadosTreinamento.txt', 'a') as arquivo:
        arquivo.write(
            str(posicaoYraquete) + " " + str(posicaoXbola) + " " + str(posicaoYbola) + " " + str(tecla) + "\n")
